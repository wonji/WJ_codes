        ...
        initAutoComplete: function (dropOption) {
            ...
            $('.quickSearchFormContainer .autocomplete').catcomplete({
                ...
                select: function (event, ui) {
                    if($(event.target).attr('name') != 'quickLinkJournalMain'){
        ...
        .done(function (resultData) {
            ...
            var getSuggestion = function(item){
                ...
                if (item.param == "SeriesKey") {
                    paramFormattedValue = "Publication";
                ...
                if (item.param === 'history') {
                    ...
                    suggestion['value'] =  ['/action/doSearch?target=default&', item.param, '=', item.value].join('');
                }
                return suggestion;
            };
            ...
            $.each(resultData, function (i, item) {
                    if (i == 0 && $inputSearchText.attr('name')!='quickLinkJournalMain') {
                        var x = {
                ...
                } else if ( (item.param === 'ContentGroupKey') && (numOfTitles < confNumOfPublicationTitles) ) {
                    results.push(getSuggestion(item));
                    numOfTitles = numOfTitles + 1;
...
grecaptcha.reset(0);};var commerce = {};
...
commerce.validators = (function() {
    ...
    instance.creditcard = function(value, element) {
        ...
        Object.keys(creditCardsPattern).forEach(function(k) {
            if (creditCardsPattern[k].test(value) && invalid) {
                invalid = false;
...
commerce.PaymentWidget.prototype.registerListeners = function () {
    ...
    $(document).ready(function () {
        ...
        if (!$(".checkout-expand > .errorMsgBox").hasClass("hidden")) {
            $('html, body').animate({
...
$(function(){var $popups=$('.popup');var $popup=$('.login-popup');var $login=$popup.find('.login');var $password=$popup.find('.password');var $eye=$popup.find('.password-eye-icon');var $remember=$popup.find('.remember .cmn-toggle');var $message=$popup.find('.message');var $submit=$popup.find('.submit');var items=$popup.find('a, button, input, select');var lastItem,revers=false,tabKey=9,shift=16,$close=$popup.find('.close');items.each(function(index){if(index===items.length-1){lastItem=$(this);}});$popup.on('keydown',function(e){if(e.keyCode===shift){revers=true;}
if((e.keyCode||e.which)===tabKey){if(!revers){tabEvent();}else{tabRevers();}}});$('.show-login').click(function(event){$popups.addClass('hidden');$popup.removeClass('hidden');$popup.find('#login').focus();$('body, html').addClass('noscroll');event.preventDefault();});$popup.on('click','.close',function(e){e.preventDefault();$('body, html').removeClass('noscroll');$popup.addClass('hidden');$eye.addClass('hidden icon-eye').removeClass('icon-eye-blocked');$submit.attr('disabled',true);$remember.attr('checked',false);$login.val('');$password.val('');$message.html('');});$popup.on('keyup',function(e){if(e.keyCode===27){$popup.find('.close').trigger("click");}
...
$(document).ready(function() {
    $(".show-request-reset-password").click(function () {
        if ($('.request-reset-password-drawer').find('.g-recaptcha ').length) {
            grecaptcha.reset();
        }else {
            $.ajax({
                type:'GET',
            ...
            }).done(function(data) { $('.password-recaptcha-ajax').append(data)})
        }
    ...
    $(".show-request-username").click(function () {
        $.ajax({
            ...
            dataType:'html',
            url:'/pb/widgets/CaptchaResponseHandler/'
        }).done(function(data) { $('.username-recaptcha-ajax').append(data)})
    })
});
