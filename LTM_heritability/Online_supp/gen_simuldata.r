#### data.generate
### Nuclear family
genNucFam <- function(totalfam,num.fammem=c(3:6),prob.fammem=c(0.2,0.3,0.3,0.2),MAF,h2,ha2,prev,n.cores=1){
	library(MASS)
	library(parallel)
	## nucelar family ##
	numfam <- sample(num.fammem,totalfam,replace=T,prob=prob.fammem)
	...
	DesignX <- function(i){
	  ...
	  for(ii in num.fammem) X[which(dat$famsize==ii)] <- Gen_offs(ii)
		return(X)
	}
