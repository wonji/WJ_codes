...
REx_WLM <- function(dataset,dep_var,indep_cat_var=NULL,indep_numeric_var=NULL,vars=NULL,weight_var=NULL,weight_option=FALSE,noint=FALSE,
	...
	html.output <- capture.output({
		...
		# Title
		R2HTML::HTML(R2HTML::as.title("Weighted Linear Regression"),HR=1,file="./test.html",append=FALSE)
		...
		if(exists('warn.msg1')|exists('warn.msg4')|exists('warn.msg5')|exists('warn.msg7')|exists('warn.DP1')|exists('warn.DP5')|exists('warn.DP6')){
			...
			if(exists('warn.msg5')) {
				if(exists('warn.msg2')) R2HTML::HTML(warn.msg2,file="./test.html")
				R2HTML::HTML(warn.msg5,file="./test.html")
			}
			if(exists('warn.msg7')) R2HTML::HTML(warn.msg7,file="./test.html")
			...
			if(class(res_LM_1)=='try-error'){
				...
				if(exists('test.dataset')&Predict_test) {
					...
					mer.1 <- mer[order(as.numeric(as.character(mer$Row.names))),]
					if(exists('O')) {
						O <- cbind(O,Predicted_test_WLM=mer.1[,3])
					} else {
						O <- data.frame(Predicted_test_WLM=mer.1[,3])
					}
				}
				if(exists('test.dataset')&Predict_CI_test){
					...
					mer <- merge(temp,temp.0,by='row.names',sort=F,all=T)
					mer.1 <- mer[order(as.numeric(as.character(mer$Row.names))),c((ncol(mer)-1),ncol(mer))]
					colnames(mer.1) <- paste0('Predicted_',round(confint.level_test*100,0),'CI_',c('Lower','Upper'),'_test_WLM')
					if(exists('O')) {
						O <- cbind(O,mer.1)
					} else {
						O <- mer.1
				...
				if(exists('test.dataset')&Predict_PI_test){
					...
					mer <- merge(temp,temp.0,by='row.names',sort=F,all=T)
					mer.1 <- mer[order(as.numeric(as.character(mer$Row.names))),c((ncol(mer)-1),ncol(mer))]
					colnames(mer.1) <- paste0('Predicted',round(confint.level_test*100,0),'PI_',c('Lower','Upper'),'_test_WLM')
					if(exists('O')) {
						O <- cbind(O,mer.1)
					} else {
						O <- mer.1
				...
				used.var <- ifelse(is.null(vars),0,length(unique(c(unlist(strsplit(vars,":")),weight_var,Part.var,Pred.var))))
				...
				## Results
				R2HTML::HTML(R2HTML::as.title("Results of Weighted Linear Regression"),HR=2,file="./test.html")
				...
				R2HTML::HTML(Digits(fitness_print), file="./test.html", align="left",digits=15,caption="<div style='text-align:left'> <li> A model with a smaller value is better.") ;
				...
				if(Plot) {
					R2HTML::HTML(R2HTML::as.title("Graphs for Regression Diagnostics"),HR=3,file="./test.html",append=TRUE)
					load.pkg("ggfortify")
		...
		R2HTML::HTML(paste("Analysis is finished at ",Sys.time(),". Rex : Weighted Linear Regression.",sep=""),file="./test.html")
		R2HTML::HTMLhr(file="./test.html")
	})
