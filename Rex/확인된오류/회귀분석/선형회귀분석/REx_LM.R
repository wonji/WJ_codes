	...
	html.output <- capture.output({
		...
		if(exists('warn.msg1')|exists('warn.msg4')|exists('warn.DP1')|exists('warn.DP5')|exists('warn.DP6')){
			...
			if(class(res_LM_1)=='try-error'){
				...
				R2HTML::HTML(R2HTML::as.title("Results of Linear Regression"),HR=2,file="./test.html")
				...
				if(length(vars)!=0){
					...
					if(Valid.method=='Partition'){
						...
						mod_fit <- caret::train(form.1,  data=newdat, method='glm',family=gaussian,trControl = ctrl, tuneLength = 5)
						A8 <- mod_fit$results
		...
		R2HTML::HTML(paste("Analysis is finished at ",Sys.time(),". Rex : Linear Regression",sep=""),file="./test.html")
		R2HTML::HTMLhr(file="./test.html")
	})
