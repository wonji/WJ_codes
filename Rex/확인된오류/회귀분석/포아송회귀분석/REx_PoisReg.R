...
REx_PoisReg <- function(dataset, res_var, quan_var=NULL, qual_var=NULL, vars=NULL, offset=NULL, link=c("log", "identity", "sqrt"), noint=FALSE, over_disp=FALSE, 
	...
	if(length(res_var) > 1) 
		Predict_train <- Predict_CI_train <- Resid <- stdResid <- studResid <- cook_distance <- linear_pred <- hat_value <- Predict_test <- Predict_CI_test <- Predict_CI_pred <- Part_index <- FALSE
		...
		html.output <- capture.output({
			...
			if(exists('warn.msg1')|exists('warn.msg4')|exists('warn.DP1')|exists('warn.DP5')|exists('warn.DP6')){
				...
				if('try-error'%in%class(fit_glm)){
					R2HTML::HTML(R2HTML::as.title("Warnings"),HR=2,file=stdout())
					R2HTML::HTML("<li> Warning : Poisson regression was failed to fit. Analysis has been stopped.",file=stdout())
					...
					if(exists('test.dataset') & Predict_test) {
						...
						mer.1 <- mer[order(as.numeric(as.character(mer$Row.names))),]
						if(exists('O')) {
							O <- cbind(O,Predicted_testing_Pois=mer.1[,3])
						} else {
							O <- data.frame(Predicted_testing_Pois=mer.1[,3])
						}
					}
					if(exists('test.dataset') & Predict_CI_test){
						...
						mer <- merge(temp,temp.0,by='row.names',sort=F,all=T)
						mer.1 <- mer[order(as.numeric(as.character(mer$Row.names))),c((ncol(mer)-1),ncol(mer))]
						colnames(mer.1) <- paste0('Predicted_',round(confint.level_test*100,0),'CI_',c('Lower','Upper'),'_testing_Pois')
					...
					used.var <- ifelse(is.null(vars),0,length(unique(unlist(strsplit(vars,":")))))+length(c(Part.var,Pred.var))+length(offset)
					...
					total.n <- paste0(nrow(original.dataset)," (Missing: ",sum(!complete.cases(original.dataset[,c(Vars,res_var,offset,Part.var,Pred.var),drop=F])),")")
					...
					if(exists('warn.offset1')) R2HTML::HTML(warn.offset1, file=stdout(), row.names=FALSE)
					if(exists('warn.offset2')) R2HTML::HTML(warn.offset2, file=stdout(), row.names=FALSE)
					...
					if(exists('warn.VS1')) R2HTML::HTML(warn.VS1,file=stdout())
					if(exists('warn.VS2')) R2HTML::HTML(warn.VS2,file=stdout())
					if(exists('warn.DP4')) R2HTML::HTML(warn.DP4,file=stdout())
					if(exists('warn.DP7')) R2HTML::HTML(warn.DP7,file=stdout())
					if(exists('warn.DP3')) R2HTML::HTML(warn.DP3,file=stdout())
					...
					## Analysis Results
					R2HTML::HTML(R2HTML::as.title("Results of Poisson Regression"),HR=2,file=stdout())
					...
					if(exists('warn.vif')) R2HTML::HTML(warn.vif, file=stdout(), row.names=FALSE)
					if(exists('warn.vip')) R2HTML::HTML(warn.vip, file=stdout(), row.names=FALSE)
					...
					if(ANOVA){
						...
						if(length(vars)==0){
							...
							R2HTML::HTML(warn.AT0,file=stdout())
						} else if(noint){
							...
							R2HTML::HTML(R2HTML::as.title(paste0("Variable Effect with Type ",ss_type," SS")),HR=4,file=stdout())
							...
							if(ss_type %in% c('II','III')){
								...
								if(class(res)[1]!='try-error'){
									...
									R2HTML::HTML(warn.AT4,file=stdout())
								}
							}
						}
					}
					...
					if(GOF){
						...
						if(length(vars)!=0){
							...
							R2HTML::HTML("<li> Warning : Goodness of fit test is not supported for intercept only model.",file=stdout())
						}
					}

					## Overdispersion Test (ref: http://127.0.0.1:15678/library/AER/html/dispersiontest.html)
					if(ODtest) {
						R2HTML::HTML(R2HTML::as.title("Test for Overdispersion"),HR=3,file=stdout(),append=TRUE)
						load.pkg('AER')
						...
						R2HTML::HTML(Digits(ODres1),file=stdout(),align="left",digits=15,row.names=F)
					}
